...
class CreateUser : Simulation() {
    ...
    private val httpProtocol =
        http.baseUrl("http://192.168.1.93:8080/bank").disableFollowRedirect().inferHtmlResources().acceptHeader("*/*")
    ...
    fun user() = NewUser(  //TODO: Implementere som feeder
        address = "string",
        country = "string",
        dob = "12/12/1990",
        firstName = "string",
    ...
    val scenario = scenario("Add User").exec(
        ...
        http("Auth User").post { session ->
            "/api/v1/auth?password=${session.get<NewUser>("userBody").password}&username=${
                session.get<NewUser>(
                    "userBody"
                ).emailAddress
            }"
        ...
        http("Create acoount").post("/api/v1/user/account").headers(genericHeader).body(StringBody { session ->
        ...
        }).header("Authorization", "Bearer \${authTokenUser}").check(bodyString().saveAs("users"))
    ).exec { session ->
        println("!!!! ${session.getString("users")}")
        session
    }
